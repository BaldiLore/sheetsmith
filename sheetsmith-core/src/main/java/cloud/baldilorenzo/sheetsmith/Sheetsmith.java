 ...
 * columns are computed on first use and cached per class, so later calls reuse them. Every call to
 * {@link #generate(List)} builds its own workbook, so concurrent calls do not interfere.
 *
 * <pre>{@code
